...
export class SolcitudComponent implements OnInit {
  ...
  FinalizarSolicitu = async () => {
    ...
    // filtrar materiales para evitar enviar cantidades nulas o <= 0
    const materialesValidos = this._materiales.filter(m => Number(m.cantidad) > 0);

    if (!materialesValidos || materialesValidos.length === 0) {
      Swal.fire({
        title: 'Error',
        text: 'Debe ingresar al menos una cantidad mayor a 0 para los materiales seleccionados',
        icon: 'error',
        showConfirmButton: false,
        timer: 3000
      });
    ...
    requisicion.producto.materiales[0] = materialesValidos;

    try {
      // validar existencias sólo para los materiales con cantidad > 0
      await Promise.all(materialesValidos.map(async (material) => {
